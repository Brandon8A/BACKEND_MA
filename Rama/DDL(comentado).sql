-- =============================================================================
-- PROYECTO: FLASH-DELIVERY MVP  (Entrega de paquetes de última milla)
-- ARCHIVO : DDL.sql
-- MOTOR   : PostgreSQL 14+
-- OBJETIVO: Modelo relacional normalizado hasta 3FN (sin violarla en ninguna
--           tabla). Cubre Gestión de Usuarios, Solicitud de Envío, Match
--           Repartidor-Pedido, Estados/Tracking, Calificaciones y catálogos
--           geográficos de Guatemala.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. CATÁLOGOS GEOGRÁFICOS (Guatemala): Departamento -> Municipio -> Zona
--    Se normalizan en 3 tablas separadas para evitar dependencias transitivas
--    (el nombre del departamento no debe repetirse en cada dirección).
-- -----------------------------------------------------------------------------
...
--    Se usan tablas catálogo en vez de texto libre o ENUM para que el negocio
--    pueda agregar valores (nuevos estados, vehículos, métodos de pago) sin
--    alterar el esquema, y para eliminar redundancia de strings repetidos.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3. USUARIOS Y ROLES  (Épica A: Gestión de Usuarios)
--    Un mismo usuario puede tener varios roles (M:N) -> tabla puente
--    usuario_roles. El perfil de repartidor vive en una tabla 1:1 aparte
--    porque sus atributos (vehículo) NO aplican a un Cliente -> evita
--    columnas nulas masivas y respeta 2FN/3FN.
-- -----------------------------------------------------------------------------
...
CREATE TABLE usuarios (
    ...
    telefono                   VARCHAR(8) NOT NULL,         -- formato GT: 8 dígitos
    dpi                         VARCHAR(13) UNIQUE,          -- Documento Personal de Identificación
...
CREATE TABLE perfiles_repartidor (
    ...
    modelo                          VARCHAR(40),
    color                             VARCHAR(20),
    calificacion_promedio              NUMERIC(3,2) NOT NULL DEFAULT 0.00, -- caché mantenido por trigger
    total_calificaciones                 INTEGER NOT NULL DEFAULT 0,        -- caché mantenido por trigger
...
CREATE TABLE direcciones (
    ...
    municipio_id                SMALLINT NOT NULL REFERENCES municipios(municipio_id),
    zona_id                       SMALLINT REFERENCES zonas(zona_id),   -- nulo si el municipio no usa zonas
    direccion_linea                 VARCHAR(200) NOT NULL,              -- Ej: "12 avenida 5-30"
    referencia                        VARCHAR(200),                     -- Ej: "Frente a Pollo Campero"
    latitud                             NUMERIC(10,7),
    longitud                              NUMERIC(10,7)
);

-- -----------------------------------------------------------------------------
-- 5. ENVÍOS  (Épica B y C: Solicitud de Envío + Match)
--    tarifa_calculada es columna GENERADA (Km * Q5.00) para garantizar
--    consistencia automática sin depender de la capa de aplicación.
-- -----------------------------------------------------------------------------
...
-- Historial de cambios de estado: trazabilidad tipo "tarjeta Kanban" del
-- pedido (Pendiente -> Aceptado -> En_Camino -> Entregado), se llena con triggers.
...
-- -----------------------------------------------------------------------------
-- 6. CALIFICACIONES  (Épica D: Finalización)
--    Relación 1:1 con envío (un envío se califica una sola vez).
-- -----------------------------------------------------------------------------
...
-- =============================================================================
-- 8. FUNCIONES Y TRIGGERS (reglas de negocio garantizadas a nivel de BD)
-- =============================================================================

-- 8.1 Mantener fecha_actualizacion automáticamente
...
-- 8.2 Validar que cliente_id tenga rol Cliente y repartidor_id tenga rol Repartidor
...
-- 8.3 Validar que solo se cree perfil de repartidor si el usuario tiene ese rol
...
-- 8.4 Registrar automáticamente el historial de cambio de estado (tracking)
...
-- 8.5 Solo permitir calificar envíos en estado "Entregado"
...
-- 8.6 Mantener el caché de calificación promedio del repartidor
...
-- 9.1 Pedidos disponibles para que un repartidor los vea/acepte (Épica C)
...
-- 9.2 Resumen completo de un envío (para detalle / tracking en la app)
...
-- =============================================================================
-- 10. COMENTARIOS DE DOCUMENTACIÓN (buenas prácticas de DBA)
-- =============================================================================
